...
function renderOfficialProductSuggestions(customerId,container,field,products){
  ...
  buttons.innerHTML='';
  const addNextSuggestion=()=>{const item=queue.shift();if(!item)return false;const button=document.createElement('button');button.type='button';button.className='treatment-suggestion-chip';button.dataset.value=item.value;button.textContent=item.label;button.onclick=()=>{field.value=[field.value.trim(),item.value].filter(Boolean).join('\n');field.dispatchEvent(new Event('input',{bubbles:true}));field.focus();button.remove();addNextSuggestion();if(!buttons.querySelector('.treatment-suggestion-chip'))buttons.innerHTML='<span class="muted">選択できる候補はありません</span>'};buttons.append(button);return true};
