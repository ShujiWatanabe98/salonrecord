...
async function openTreatmentEntryForm(customerId='',anchor=null){
  ...
  try{
    ...
    const refreshSuggestions=()=>{loadTreatmentSuggestions(customerSelect.value,suggestionBox,treatmentField);loadProductSuggestions(customerSelect.value,productSuggestionBox,productsField);loadFootTreatmentSuggestions(customerSelect.value,footTreatmentSuggestions,footTreatmentField);loadFootProductSuggestions(customerSelect.value,footProductSuggestions,footProductsField);loadBodyTreatmentSuggestions(customerSelect.value,bodyTreatmentSuggestions,bodyTreatmentField);loadBodyProductSuggestions(customerSelect.value,bodyProductSuggestions,bodyProductsField)};
    customerSelect.addEventListener('change',refreshSuggestions);if(customerSelect.value)refreshSuggestions();
    ...
    const form=dialog.querySelector('form');form.onsubmit=async event=>{event.preventDefault();const submit=form.querySelector('[type="submit"]'),error=form.querySelector('.treatment-entry-error'),values=Object.fromEntries(new FormData(form));if(!values.customerId){error.textContent='お客様を選択してください';return}submit.disabled=true;error.textContent='';try{const template=state.templates[0];if(!template)throw new Error('保存用フォームがありません');const images=[...dialog.querySelectorAll('.treatment-drawing-canvas')].map(canvas=>canvas.toDataURL('image/png'));await api('/api/records',{method:'POST',body:JSON.stringify({customerId:values.customerId,visitDate:values.visitDate,templateId:template.id,recordType:'treatment_entry',values:{treatment:values.treatment||'LYCON施術',treatmentDetails:values.treatmentDetails,productsUsed:values.productsUsed,skinCondition:values.skinCondition,cautions:values.cautions,retail:values.retail,discount:values.discount,amount:values.amount,discountedAmount:values.discountedAmount},alerts:values.cautions?[values.cautions]:[],note:values.comment,images})});state.selectedCustomerId=values.customerId;toast('LYCON施術記録を保存しました');close();show('dashboard')}catch(saveError){error.textContent=saveError.message;submit.disabled=false}};
...
async function loadTreatmentSuggestions(customerId,container,field){
  renderOfficialProductSuggestions(customerId,container,field,['フェイシャルWaxingのアプリケーション','アイブロウの形と整え方','施術前後のカウンセリング']);
}
async function loadProductSuggestions(customerId,container,field){
  renderOfficialProductSuggestions(customerId,container,field,['ライコジェット アイブロウワックス 500g','ライコテック ホワイトハードワックス 500g','アフターワックスローションS （ミルクタイプ）500ml']);
}
async function loadFootTreatmentSuggestions(customerId,container,field){
  const fallbacks=['リフレッシングソークを使ったフットバス','カルスリムーバーとフットパドルによる角質ケア','シュガースクラブとマッサージングローションによる保湿ケア'];
...
async function loadFootProductSuggestions(customerId,container,field){
  const fallbacks=["lyco'pedi リフレッシングソーク 500ml","lyco'pedi カルスリムーバー 100ml","lyco'pedi スムージングクリーム 250ml"];
  renderOfficialProductSuggestions(customerId,container,field,fallbacks);
}
async function loadBodyProductSuggestions(customerId,container,field){
  const fallbacks=['アクティブゴールド ストリップワックス 800g','ライコテック ホワイトストリップワックス800g','アフターワックスジェル500ml'];
  renderOfficialProductSuggestions(customerId,container,field,fallbacks);
}
async function loadBodyTreatmentSuggestions(customerId,container,field){
  const fallbacks=['ボディWaxingのアプリケーション','ストリップワックスとハードワックスの使い分け','施術前後のカウンセリング'];
  renderOfficialProductSuggestions(customerId,container,field,fallbacks);
}
function renderOfficialProductSuggestions(customerId,container,field,products){
  ...
  buttons.innerHTML=products.map(value=>`<button type="button" class="treatment-suggestion-chip" data-value="${esc(value)}">${esc(value)}</button>`).join('');
  buttons.querySelectorAll('.treatment-suggestion-chip').forEach(button=>button.onclick=()=>{field.value=button.dataset.value;field.focus();buttons.querySelectorAll('button').forEach(item=>item.classList.toggle('selected',item===button))});
}
