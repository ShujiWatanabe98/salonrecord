...
async function saveOcrRecord(frame,button){
  ...
  try{
    ...
    await api('/api/records',{method:'POST',body:JSON.stringify({customerId:customer.id,visitDate,templateId:template.id,chartGender:state.chartGender,values,alerts:[],note:`${chartLanguageLabel(state.chartLanguage)} ${genderLabel(state.chartGender)} ${treatmentLabel} OCRから保存`,images})});
