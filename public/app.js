...
const state = { token: sessionStorage.getItem('salonToken'), user: null, customers: [], templates: [], scan: null, image: '', images: [], partImages: { new: '', progress: '' }, treatmentType: 'フット', chartGender: 'common', chartLanguage: 'ja', uiLanguage: requestedUiLanguage==='en'||requestedUiLanguage==='ja'?requestedUiLanguage:localStorage.getItem('salonUiLanguage') === 'en' ? 'en' : 'ja', workflowStage: 'new', selectedTemplate: null, sheetValidationToken: 0 };
const englishText = new Map(Object.entries({
  ...
  'AIカルテ':'AI Chart','カメラ':'Camera','ファイルを開く':'Open files','OCR':'OCR','保存':'Save','閉じる':'Close','キャンセル':'Cancel','撮影':'Capture','カルテ撮影':'Sheet capture','A4用紙を枠内に合わせてください':'Align the A4 sheet inside the frame',
...
function applyUiLanguage(){document.documentElement.lang=state.uiLanguage;document.querySelectorAll('.language-switch button').forEach(button=>button.classList.toggle('active',button.dataset.language===state.uiLanguage));translateUi()}
